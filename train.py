...
from helpers import setup_camera, l1_loss_v1, l1_loss_v1_w_mask, l1_loss_v2, weighted_l2_loss_v1, weighted_l2_loss_v2, quat_mult, \
    o3d_knn, params2rendervar, params2cpu, save_params
from external import calc_ssim, calc_psnr, calc_psnr_w_mask, build_rotation, densify, update_params_and_optimizer
...
def get_dataset(t, md, seq):
    ...
    for c in range(len(md['fn'][t])):
        ...
        cam = setup_camera(w, h, k, w2c, near=1.0, far=100) # cam's far is 100 so we cannot fit more than 100        
        # generate 3 pseudo camera for each camera
        # perturb the w2c 
...
def initialize_params(seq, md):
    ...
    return params, variables # params is in optim, var is not 


def initialize_optimizer(params, variables):
    lrs = {
        'means3D': 0.016 * variables['scene_radius'],
        'rgb_colors': 0.0025,
        'seg_colors': 0.0,
...
def get_loss(params, curr_data, variables, is_initial_timestep, count):
    losses = {}
    ...
    im, radius, depth, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar) 
...
def report_progress(params, data, i, progress_bar, loss, ld, li, me, every_i=1):
    if i % every_i == 0:
        im, _, depth, _ = Renderer(raster_settings=data['cam'])(**params2rendervar(params))
        curr_id = data['id']
        im = torch.exp(params['cam_m'][curr_id])[:, None, None] * im + params['cam_c'][curr_id][:, None, None]
        # psnr = calc_psnr(im, data['im']).mean()
        psnr = calc_psnr_w_mask(im, data['im'], data['mask']==0).mean()
        progress_bar.set_postfix({"me": f"{me:.{7}f}", "ld": f"{ld:.{7}f}", "li": f"{li:.{7}f}", "psnr": f"{psnr:.{7}f}"})
...
def train(seq, exp):
    ...
    for t in range(2):
        ...
        num_iter_per_timestep = 10000 if is_initial_timestep else 500
        progress_bar = tqdm(range(num_iter_per_timestep), desc=f"timestep {t}")
        for i in range(num_iter_per_timestep):
            curr_data = get_batch(todo_dataset, dataset)
            loss, variables, ld, li, me = get_loss(params, curr_data, variables, is_initial_timestep, i)
            loss.backward()
            with torch.no_grad():
                report_progress(params, dataset[0], i, progress_bar, loss, ld, li, me)
